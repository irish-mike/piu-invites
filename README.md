...
**Status: Stage 7 — local readiness and QA. Development is local only.**
The event page submits full name and email through a server-validated endpoint to Formspark. Confirmed success runs a blackout and an animated jump scare with a shifting, flickering entrance and exit before settling on the final copy; reduced motion skips the effect. Signed cookies prevent repeat submissions of a confirmed email from the same browser for 180 days; recognized repeats show confirmation directly. A server-rendered interest message shows rounded milestones from unique emails, hidden below 30. It refreshes roughly every five minutes and disappears if retrieval fails. A server-enforced honeypot provides basic spam protection. Stronger rate limiting, analytics, and deployment remain deferred. Real Formspark submission, read access, and confirmation-email delivery require configuration and live verification.
...
The form uses native HTML controls; shadcn/ui is not needed.
...
Set `FORMSPARK_FORM_ID` and `REGISTRATION_COOKIE_SECRET` in `.env.local` to enable registration, then restart the server. Generate a random cookie secret with `node -e "console.log(require('node:crypto').randomBytes(32).toString('hex'))"` and keep it stable across restarts. No API token is needed for submission, and these variables must not use a `NEXT_PUBLIC_` prefix. Missing configuration keeps the page usable but submissions return a recoverable error before contacting Formspark. To preview the success animation without submitting to Formspark, run `REGISTRATION_PREVIEW=true npm run dev`, or set `REGISTRATION_PREVIEW=true` in `.env.local` and restart the development server. Preview submissions return success immediately, including with empty fields, and skip validation, the honeypot, and cookies. This setting is ignored in production (`npm start`). Refresh the page to replay. See [registration setup](docs/registration.md) for Formspark dashboard configuration and the live verification checklist.
...
Tests use Node’s built-in runner with `tsx` for TypeScript. They cover registration, success state transitions including reduced motion, unique-email counting, milestone boundaries, and Formspark pagination/failures. External HTTP is replaced; tests never access a real Formspark account. See [registration verification](docs/registration.md#verification) for visual and accessibility checks and [local QA](docs/local-qa.md) for the completed checks and remaining live verification.
